...
DragginatorInfosFromDnaResponse dragginatorInfosFromDnaResponseFromJson(
        String str) =>
    DragginatorInfosFromDnaResponse.fromJson(json.decode(str));

String dragginatorInfosFromDnaResponseToJson(
        DragginatorInfosFromDnaResponse data) =>
...
class DragginatorInfosFromDnaResponse {
  ...
  bool isAgeOkToRegisterTheHunt() {
    try {
      if (this.status!.toLowerCase() != "egg") {
        return false;
      }
      double _birth = 1000 * double.tryParse(this.birth!)!;
      DateTime dateBirth = DateTime.fromMillisecondsSinceEpoch(_birth.toInt());
      int? age = DateTime.now().difference(dateBirth).inDays;
      if (age >= 30) {
        return true;
      } else {
        return false;
